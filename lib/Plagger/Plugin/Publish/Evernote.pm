package Plagger::Plugin::Publish::Evernote;
use strict;
...
sub notify {
    ...
    my $body = $self->templatize('evernote_notify.tt', { feed => $feed, encoding => $encoding });
    ...
    if ($self->conf->{notebook}) {
        $subject = encode('MIME-Header', $subject) . " @" . $self->conf->{notebook};
    } else {
...
Plagger::Plugin::Publish::Evernote - Notify updates to your Evernote via mail
...
This plugin creates HTML emails and sends them to your Evernote.
...
Notebook name to send updates to.
